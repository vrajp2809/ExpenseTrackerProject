...
export const UserExpenseSection = () => {
  ...
  useEffect(() => {
    const fetchExpenses = async () => {
      try {
        ...
        const expenseTransactions = response.data.data
  ...
  useEffect(() => {
    ...
    }, [category, search, startDate, endDate, expenses]);

  // Function to handle delete
  const handleDelete = async (id) => {
    try {
      await axios.delete("/deleteExpense/" + id);
      setExpenses(expenses.filter((transaction) => transaction._id !== id));
    } catch (error) {
  ...
  const columns = [
    { field: "paidTo", headerName: "Paid To", flex: 1.2, minWidth: 150 },
        ...
        <DialogContent>
          {selectedExpense && (
            <Box sx={{ padding: "10px", fontSize: "16px" }}>
              <Typography>
                <strong>Paid To:</strong> {selectedExpense.paidTo}
              </Typography>
              <Typography>
                <strong>Category:</strong> {selectedExpense.category}
              </Typography>
              <Typography>
                <strong>Amount:</strong> ₹{selectedExpense.amountSpent}
              </Typography>
              <Typography>
                <strong>Transaction Type:</strong> Expense
              </Typography>
              <Typography>
                <strong>Date:</strong>{" "}
                {new Date(selectedExpense.transactionDate).toLocaleDateString()}
              </Typography>
              <Typography>
                <strong>Notes:</strong> {selectedExpense.notes || "N/A"}
              </Typography>
